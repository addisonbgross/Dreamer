...
public class Library {
	...
	static Object get(String s, FileType ft) {
		if (ft == FileType.IMG) {
			return getImage(s);
			/*
			 * static void loadImage(String file) { String referenceName =
			 * file.substring(0, file.toString().lastIndexOf("."));
	...
	/**
	 * Gather a list of points within the specified Obj file where new light
	...
	static ArrayList<Vector3f> getModelLights(String s, int scale, int x,
			int y, int z) {
		ArrayList<Vector3f> lightPointList = null;
		try {
			lightPointList = ObjLoader
					.loadLights(models.get(s), scale, x, y, z);
		} catch (FileNotFoundException e) { // model not found
			System.out.println("Model [" + s + "] not found!");
			e.printStackTrace();
		} catch (IOException e) {// model not found in a different way
	...
	static void loadImage(String file) {
		String referenceName = file.substring(0,
				file.toString().lastIndexOf("."));

		ImageTracker tempImage = new ImageTracker(referenceName + ".png");
		referenceName = referenceName.replace("backgrounds\\", "").replace(
				"backgrounds/", "");
	...
	static void importFonts() {
		...
		Font chunk;
		try {
			inputStream = ResourceLoader.getResourceAsStream(Constants.RESPATH
					+ Constants.FONTSPATH + "8-bit Madness.ttf");
...
/*
 * ImageTracker: this keeps a scaled reference of an Image and updates this as
 * necessaryTODO probably should switch all Images besides backgrounds and
 * foregrounds to textures
 */
class ImageTracker {
	...
	ImageTracker(String s) {
		try {
			...
			if (Library.messages)
				System.out.println("Image import of " + s + ".png successful");
